import { useState } from 'react';

function GoogleIcon() {
  return (
    <svg viewBox="0 0 24 24" width="20" height="20">
      <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04-2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
      <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
      <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
      <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
    </svg>
  );
}

function GitHubIcon() {
  return (
    <svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor">
      <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z"/>
    </svg>
  );
}

function handleGoogleLogin() {
  window.location.href = '/api/auth/oauth/google';
}

function handleGitHubLogin() {
  window.location.href = '/api/auth/oauth/github';
}

export function AuthPage({ onLogin, onRegister, submitting, error }) {
  const [mode, setMode] = useState('login');
  const [formState, setFormState] = useState({
    displayName: '',
    email: '',
    password: ''
  });

  const handleSubmit = (event) => {
    event.preventDefault();
    if (mode === 'login') {
      onLogin({ email: formState.email, password: formState.password });
      return;
    }
    onRegister(formState);
  };

  return (
    <div className="auth-screen">
      <div className="auth-panel auth-left">
        <div className="auth-left-content">
          <div className="auth-logo-wrap">
            <img src="/nextagent-logo.png" alt="NextAgent" className="auth-logo" />
          </div>

          <h1 className="auth-hero-title">
            Crea hoy los{' '}
            <span className="accent-cyan">agentes</span>
            {' '}que transformarán el{' '}
            <span className="accent-blue">mañana.</span>
          </h1>

          <p className="auth-hero-sub">
            NextAgent es tu plataforma para construir agentes de IA con identidad,
            memoria y acceso a herramientas{' '}
            <span className="accent-pink">reales.</span>
          </p>
        </div>

        <div className="brand-personality">
          <div className="brand-personality-grid">
            <div className="brand-trait">
              <svg className="brand-trait-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round">
                <path d="M4.5 16.5c-1.5 1.26-2 5-2 5s3.74-.5 5-2a2.12 2.12 0 0 0-3-3Z"/>
                <path d="M12 15 7 9a9 9 0 0 1 7-7 9 9 0 0 1 2 8"/>
                <path d="m15 9 3.5 3.5a9 9 0 0 1-8 2"/>
              </svg>
              <strong>Innovadora</strong>
            </div>
            <div className="brand-trait">
              <svg className="brand-trait-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round">
                <path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/>
                <path d="m9 12 2 2 4-4"/>
              </svg>
              <strong>Confiable</strong>
            </div>
            <div className="brand-trait">
              <svg className="brand-trait-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round">
                <path d="M15 14c.2-1 .7-1.7 1.5-2.5 1-.9 1.5-2.2 1.5-3.5A6 6 0 0 0 6 8c0 1 .2 2.2 1.5 3.5.7.7 1.3 1.5 1.5 2.5"/>
                <path d="M9 18h6"/><path d="M10 22h4"/>
              </svg>
              <strong>Inteligente</strong>
            </div>
            <div className="brand-trait">
              <svg className="brand-trait-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round">
                <circle cx="18" cy="5" r="3"/><circle cx="6" cy="12" r="3"/><circle cx="18" cy="19" r="3"/>
                <line x1="8.59" y1="13.51" x2="15.42" y2="17.49"/>
                <line x1="15.41" y1="6.51" x2="8.59" y2="10.49"/>
              </svg>
              <strong>Conectada</strong>
            </div>
            <div className="brand-trait">
              <svg className="brand-trait-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round">
                <path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/>
              </svg>
              <strong>Humana</strong>
            </div>
          </div>
        </div>

        <small className="auth-footer-copy">© 2026 NextAgent · Academic edition · EAFIT</small>
      </div>

      <div className="auth-panel auth-right">
        <div className="eyebrow">
          <span className="eyebrow-line" />
          CENTRO DE OPERACIONES
        </div>
        <h2>{mode === 'login' ? 'Bienvenido de nuevo' : 'Crea tu espacio de trabajo'}</h2>
        <p>
          {mode === 'login'
            ? 'Inicia sesión para gestionar tu portafolio de bots en NextAgent.'
            : 'Crea una cuenta para empezar a construir agentes inteligentes.'}
        </p>

        <form className="auth-form" onSubmit={handleSubmit}>
          {mode === 'register' ? (
            <label>
              <span>Nombre a mostrar</span>
              <input
                value={formState.displayName}
                onChange={(event) =>
                  setFormState({ ...formState, displayName: event.target.value })
                }
                required
              />
            </label>
          ) : null}

          <label>
            <span>Correo electrónico</span>
            <input
              type="email"
              value={formState.email}
              onChange={(event) => setFormState({ ...formState, email: event.target.value })}
              required
            />
          </label>

          <label>
            <span>Contraseña</span>
            <input
              type="password"
              value={formState.password}
              onChange={(event) => setFormState({ ...formState, password: event.target.value })}
              required
            />
          </label>

          {error ? <div className="error-banner">{error}</div> : null}

          <button className="primary-button" disabled={submitting}>
            {submitting ? 'Cargando...' : mode === 'login' ? 'Iniciar sesión' : 'Crear cuenta'}
          </button>
        </form>

        <div className="auth-divider">
          <span className="auth-divider-line" />
          <span className="auth-divider-text">o continúa con</span>
          <span className="auth-divider-line" />
        </div>

        <div className="oauth-buttons">
          <button type="button" className="oauth-button google" onClick={handleGoogleLogin} disabled={submitting}>
            <GoogleIcon />
            <span>Google</span>
          </button>
          <button type="button" className="oauth-button github" onClick={handleGitHubLogin} disabled={submitting}>
            <GitHubIcon />
            <span>GitHub</span>
          </button>
        </div>

        <button className="text-button" onClick={() => setMode(mode === 'login' ? 'register' : 'login')}>
          {mode === 'login' ? '¿No tienes cuenta? Regístrate gratis' : '¿Ya tienes cuenta? Inicia sesión'}
        </button>
      </div>
    </div>
  );
}
